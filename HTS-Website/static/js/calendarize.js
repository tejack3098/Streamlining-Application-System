    ...
    return {
		...
		buildMonth: function(monthNum, year, opts) {
			...
			daysInMonth.forEach(function(c, d) {
                ...
                //console.log("Day Added: "+$dayNode.getAttribute("data-date"))
                var Month = $dayNode.getAttribute("data-date").slice(4,7);
                var Day = $dayNode.getAttribute("data-date").slice(8,10)
                var Year = $dayNode.getAttribute("data-date").slice(11,15)
                //console.log($dayNode.getAttribute("data-date"))
                
                var holiday = Year+'-'+monthsNumber[Month]+'-'+Day;
                if (holiday in Holidays)
                {   
                    $dayNode.style = "background-color:#80dfff;";
                    $dayNode.setAttribute("date-selected", "yes");
                    $dayNode.classList.add('holiday');// for holiday css
                    $dayNode.innerHTML = (d + 1);
                    
                    $dayNode.setAttribute("data-toggle", "tooltip");
                    $dayNode.setAttribute("title", Holidays[holiday]);

                    console.log("Chuuuttii");
                }
                else{
                    $dayNode.style = "background-color:white;";
                    $dayNode.setAttribute("date-selected", "no");
                    $dayNode.innerHTML = (d + 1);
                }
				$monthNode.appendChild($dayNode);
			});
